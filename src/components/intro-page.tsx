...
import { Sparkles, Users, FileText, Brain, History, Database, Network, Cpu } from "lucide-react";

export const IntroPage = () => {
  // ✅ FIX 1: Handle initial page load with a hash in the URL
  useEffect(() => {
  ...
  // ConvexClientProvider already ensures this page ONLY loads for Unauthenticated users.
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900 text-gray-900 dark:text-gray-100 overflow-x-hidden">
      ...
      <section className="relative w-full min-h-[85vh] flex flex-col lg:flex-row items-center justify-center lg:justify-between px-4 sm:px-8 md:px-12 lg:px-20 py-12 sm:py-20 overflow-hidden gap-12 lg:gap-0">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs sm:text-sm font-medium mb-2 border border-blue-200 dark:border-blue-800">
          ...
          >
            {[
              ...
              { title: "AI Assistant:", text: "Grammar, tone, and RAG context generation." },
            ].map((item, index) => (
              <motion.li
                key={index}
                variants={{
              ...
              >
                <div className="mt-1.5 size-1.5 sm:size-2 rounded-full bg-blue-500 shrink-0" />
                <p>
        ...
        {/* Right Content (SignIn Box) */}
        <motion.div 
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.8, delay: 0.2 }}
          className="w-full sm:max-w-md lg:w-auto z-10"
        >
          <div className="p-4 sm:p-6 md:p-8 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl rounded-2xl sm:rounded-3xl shadow-[0_8px_40px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_40px_rgba(0,0,0,0.3)] border border-gray-200/60 dark:border-gray-700/60 transform transition-all duration-500 hover:shadow-2xl w-full">
            <SignIn
              routing="hash"
              appearance={{
                elements: {
                  ...
                  socialButtonsBlockButton: "rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition",
            ...
            />
            <p className="text-[10px] sm:text-[11px] text-center text-gray-400 dark:text-gray-500 mt-4 uppercase tracking-wider font-semibold">
      ...
      <section id="how-it-works" className="px-4 sm:px-6 py-16 sm:py-24 bg-gray-50 dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800 scroll-mt-10">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 relative">
            <div className="hidden lg:block absolute top-1/2 left-10 right-10 h-0.5 bg-gradient-to-r from-blue-200 via-purple-200 to-blue-200 dark:from-gray-700 dark:via-gray-600 dark:to-gray-700 -z-10"></div>
...
const FeatureCard = ({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) => (
  <div className="p-6 sm:p-8 border border-gray-100 dark:border-gray-800 rounded-xl sm:rounded-2xl bg-gray-50/50 dark:bg-gray-800/30 hover:bg-white dark:hover:bg-gray-800 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group flex flex-col items-start">
    <div className="mb-4 sm:mb-5 inline-flex p-2.5 sm:p-3 rounded-lg bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 group-hover:scale-110 transition-transform">
      {icon}
    </div>
...
const StepCard = ({ number, icon, title, desc }: { number: string, icon: React.ReactNode, title: string, desc: string }) => (
  <div className="p-6 sm:p-8 border border-gray-100 dark:border-gray-700 rounded-xl sm:rounded-2xl bg-white dark:bg-gray-800 shadow-sm relative z-10 flex flex-col items-center hover:shadow-lg transition-shadow text-center">
    <div className="absolute -top-3 -left-3 sm:-top-4 sm:-left-4 text-4xl sm:text-5xl font-black text-gray-100 dark:text-gray-700/50 -z-10 select-none">
      {number}
    </div>
    <div className="mb-4 sm:mb-6 p-3 sm:p-4 rounded-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700">
      {icon}
    </div>
    <h3 className="font-bold text-lg sm:text-xl mb-2 sm:mb-3 text-gray-900 dark:text-gray-100">{title}</h3>
