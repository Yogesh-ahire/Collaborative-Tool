...
export const Editor = ({ initialContent, documentId }: EditorProps) => {
    ...
    const editor = useEditor({
        ...
        extensions: [
            liveblocks, StarterKit.configure({ history: false }),
            LineHeightExtension.configure({ types: ["heading", "paragraph"], defaultLineHeight: "normal" }),
            FontSizeExtension, TextAlign.configure({ types: ["heading", "paragraph"] }),
            Link.configure({ openOnClick: false, defaultProtocol: "https" }),
            Color, Highlight.configure({ multicolor: true }), FontFamily, TextStyle, Underline,
            ImageResize, Table, TableCell, TableHeader, TableRow,
            TaskItem.configure({ nested: true }), TaskList,
        ],
    });
                ...
                if (pendingChanges.current >= 120 || now - lastSaveTime.current >= 300000) {
                    try {
                        ...
                        if (plainText.trim().length > 0) {
                            ...
                            }).catch(err => console.error("Failed embedding:", err));
                        }
        ...
        const syncTelemetry = async () => {
            ...
            try {
                ...
                if (data && data.ydoc) {
                    ...
                    // 🔥 CRITICAL: Force serialization to purge `undefined` fields created by minifier bugs
                    await updateTelemetry({
                        ...
                        rawNodes: JSON.parse(JSON.stringify(resolvedNodes.slice(-50))), 
                    });
                }
            } catch (err) {
    ...
    }, [editor, documentId, updateTelemetry]);
    ...
    const handleAiAction = async (action: "grammar" | "tone" | "translate", modifier?: string) => {
        ...
        try {
            const res = await fetch("/api/ai", {
                method: "POST", headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ text: selectedHTML, action: action, modifier: modifier }),
            });
    ...
    const acceptAiPreview = () => {
        if (previewContent && editor) {
            editor.chain().focus().setTextSelection({ from: previewContent.from, to: previewContent.to })
                .unsetHighlight().insertContent(previewContent.html).run();
    ...
    const discardAiPreview = () => {
        if (aiProcessingRange && editor) {
            editor.chain().setTextSelection({ from: aiProcessingRange.from, to: aiProcessingRange.to }).unsetHighlight().run();
        }
        setPreviewContent(null);
        setAiProcessingRange(null);
    };
    ...
    return (
        <div className="size-full overflow-x-auto bg-[#F9FBFD] px-4 print:p-0 print:bg-white print:overflow-visible relative">
            ...
            <div className="min-w-max flex justify-center w-[816px] py-4 print:py-0 mx-auto print:w-full print:min-w-0 relative">
                {editor && (
                    <BubbleMenu editor={editor} tippyOptions={{ duration: 100, placement: 'top' }} className="flex items-center gap-1 p-1.5 bg-white border border-gray-200 shadow-xl rounded-lg">
                        <div className="flex items-center px-2 border-r border-gray-200">
                            <Sparkles className="size-4 text-purple-600" />
                        </div>
                        <button onMouseDown={(e) => e.preventDefault()} onClick={() => handleAiAction("grammar")} disabled={isAiLoading} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 rounded-md transition disabled:opacity-50">
                            {isAiLoading ? <Loader2 className="size-3 animate-spin" /> : <SpellCheck2 className="size-3" />} Fix Grammar
                        </button>
                        <div className="relative flex items-center group">
                            <select disabled={isAiLoading} onChange={(e) => { if (e.target.value) handleAiAction("tone", e.target.value); e.target.value = ""; }} className="appearance-none bg-transparent hover:bg-gray-100 text-gray-700 font-medium text-xs px-2.5 py-1.5 pr-7 rounded-md outline-none cursor-pointer disabled:opacity-50 transition" defaultValue="">
                        ...
                        <div className="relative flex items-center group">
                            <select disabled={isAiLoading} onChange={(e) => { if (e.target.value) handleAiAction("translate", e.target.value); e.target.value = ""; }} className="appearance-none bg-transparent hover:bg-gray-100 text-gray-700 font-medium text-xs px-2.5 py-1.5 pr-7 rounded-md outline-none cursor-pointer disabled:opacity-50 transition" defaultValue="">
            ...
            {previewContent && typeof window !== "undefined" && createPortal(
                    ...
                    <div className="bg-white w-full max-w-2xl rounded-xl shadow-2xl flex flex-col overflow-hidden border border-gray-200 m-4 animate-in fade-in zoom-in-95 duration-200">
                        <div className="px-4 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            <h3 className="font-semibold text-gray-800 flex items-center gap-2">
                                <Sparkles className="size-4 text-purple-600" /> AI Preview
                            </h3>
                        ...
                        <div className="p-6 max-h-[60vh] overflow-y-auto bg-white">
                            <div className="max-w-none break-words space-y-2 [&_ul]:list-disc [&_ul]:ml-4 [&_ul]:my-1 [&_ol]:list-decimal [&_ol]:ml-4 [&_ol]:my-1 [&_strong]:font-bold [&_h3]:font-bold [&_h3]:text-base [&_h2]:font-bold [&_h2]:text-lg [&_p]:m-0 text-gray-700 text-sm" dangerouslySetInnerHTML={{ __html: previewContent.html }} />
                        </div>
                        <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                            <button onClick={discardAiPreview} className="px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                Discard
                            </button>
                            <button onClick={acceptAiPreview} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition shadow-sm">
                                Accept & Replace
                            </button>
                        </div>
                    </div>
